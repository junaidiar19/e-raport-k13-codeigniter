  ...
  text-align: center;
   ...
   <table>
    ...
    <table>
      ...
      <tr>
        <td colspan="3"><p><b>Nilai (KI3) Pengetahuan Kelas <?=$k->nm_kelas?></b></p></td>
      </tr>
    <?php }?>
...
foreach ($siswa as $sw) {
    ...
    $nph = $this->db->query("SELECT AVG(ph) as total_ph FROM tb_nki3 WHERE siswa=" . $sw->id_siswa . " AND kelas=" . $id . " AND ta=" . $ta . " AND mapel=" . $mp . " ")->result();

    $nuts = $this->db->query("SELECT AVG(npts) as total_uts FROM tb_nki3 WHERE siswa=" . $sw->id_siswa . " AND kelas=" . $id . " AND ta=" . $ta . " AND mapel=" . $mp . " ")->result();

    $nuas = $this->db->query("SELECT AVG(npas) as total_uas FROM tb_nki3 WHERE siswa=" . $sw->id_siswa . " AND kelas=" . $id . " AND ta=" . $ta . " AND mapel=" . $mp . " ")->result();

    $jum = $this->db->query("SELECT (SELECT AVG(ph) FROM tb_nki3 WHERE siswa=" . $sw->id_siswa . " AND kelas=" . $id . " AND ta=" . $ta . " AND mapel=" . $mp . ") + (SELECT AVG(npts) FROM tb_nki3 WHERE siswa=" . $sw->id_siswa . " AND kelas=" . $id . " AND ta=" . $ta . " AND mapel=" . $mp . ") + (SELECT AVG(npas) FROM tb_nki3 WHERE siswa=" . $sw->id_siswa . " AND kelas=" . $id . " AND ta=" . $ta . " AND mapel=" . $mp . ") as jumlah")->result();
